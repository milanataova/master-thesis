#!/usr/bin/env python3
"""
Generate Scatterplots with Ensemble Distributions
...
ENSEMBLE_DISTRIBUTIONS_FILE = os.path.join(BASE_DIR, "Outputs", "metrics_tables", "ensemble", "ensemble_distributions.csv")
...
def load_ensemble_distributions(filepath):
    ...
    for (country, model), group in df.groupby(["Country", "Model"]):
        ...
        probs = group.set_index("Score")["Ensemble_Prob"]
...
def weighted_relative_frequency(series: pd.Series, weights: pd.Series) -> pd.Series:
    ...
    Parameters:
        series: Life satisfaction scores
        weights: Corresponding weights

    Returns:
        pd.Series with index 1-10 and weighted relative frequencies
    """
...
def main():
    ...
    print("="*80)
    print("Life Satisfaction Distribution Scatterplots with Ensemble")
    print("="*80)
    ...
    print("\nStep 3: Loading ensemble distributions...")
    ensemble_data = load_ensemble_distributions(ENSEMBLE_DISTRIBUTIONS_FILE)
    print(f"✅ Loaded ensemble data for {len(ensemble_data)} (country, model) combinations")
    ...
    # Step 5: Create plots
    print("\nStep 5: Creating scatterplots with ensemble...")
    ...
    for country in TARGET_COUNTRIES:
        ...
        fig.suptitle(
            f"Life Satisfaction Distributions with Ensemble – {country}",
            fontsize=20,
            y=0.98,
        )
        ...
        for ax, model in zip(axes, MODELS):
            ...
            if (country, model) in ensemble_data:
                ...
                ax.plot(
                    ...
                    y_ens.values,
                    marker="o",
                    linestyle=s_ens["linestyle"],
                    linewidth=s_ens["linewidth"],
                    markersize=s_ens["markersize"],
                    color=s_ens["color"],
                    label="Ensemble",
                )
        ...
        out_path = os.path.join(PLOTS_OUTPUT_DIR, f"scatter_with_ensemble_{country}.png")
    ...
    print("\n" + "="*80)
    print("✅ All scatterplots with ensemble complete!")
